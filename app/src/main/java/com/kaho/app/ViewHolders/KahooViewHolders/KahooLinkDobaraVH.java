...
public class KahooLinkDobaraVH extends RecyclerView.ViewHolder {
    ...
    public KahooLinkDobaraVH(@NonNull KahooDobaraSingleLayoutBinding kahooDobaraBinding) {
    ...
    public void setUpKahooView(KahooLinkDobaraVH holder, Fragment mContext,
        ...
        if (dataKahoPost.getOther_kaho_data()!=null&&(dataKahoPost.isHas_dobara_kaho()||dataKahoPost.isHas_reply_kaho())){
            ...
            if (dataKahoPost.isHas_reply_kaho()){
                content="Reply to";
                //kahooSingleLayoutViewBinding.replyText.setText("Reply to @"+dataKahoPost.getOther_kaho_data().getKaho_added_user_handel_id());
            ...
            if (dataKahoPost.isHas_dobara_kaho()){
                ...
                //kahooSingleLayoutViewBinding.replyText.setText("Dobara Kahoo @"+dataKahoPost.getOther_kaho_data().getKaho_added_user_handel_id());
            }
        ...
        if (urlFromcont.length()>0){
            ...
            final SourceContent[] linkSources = new SourceContent[1];
            ...
            LinkPreviewCallback linkPreviewCallback = new LinkPreviewCallback() {
                ...
                public void onPos(SourceContent sourceContent, boolean b) {
                    linkSources[0] =sourceContent;
                    // Populate your preview layout with the results of sourceContent.
                    Log.e("jsfldkfsf","this is content title "+sourceContent.getImages().get(0));
                    int i=0;
                    for( i=0;i<sourceContent.getImages().size();i=i){
                        if (sourceContent.getImages().get(i).contains(".jpg")||
                                sourceContent.getImages().get(i).contains(".jpeg")||
                                sourceContent.getImages().get(i).contains(".png")){
                            Log.e("kdshfksjsf","break point source image is "+sourceContent.getImages().get(i));
                            break;
                        }
                        i++;
                    }
                    kahooDobaraBinding.previewTitle.setText(sourceContent.getTitle());
                        ...
                        try {
                            if (mContext!=null){
                                Glide.with(mContext)
                                        ...
                                        .error(R.color.dark_primary_500)
                                        .override(800, 560)
                                        .into(kahooDobaraBinding.linkPreview);
                            }
                        }catch (Exception e){
                            Log.e("kshfkjssds","this is error "+e.getMessage());
                        }
                    }
                    kahooDobaraBinding.linkPreviewLayout.setVisibility(View.VISIBLE);
                }
            };
            textCrawler.makePreview( linkPreviewCallback, urlFromcont);
            String finalUrlFromcont = urlFromcont;
            if (PublicMethods.isYoutubeUrl(urlFromcont)||urlFromcont.contains(".mp4")||urlFromcont.contains(".3gp")){
                kahooDobaraBinding.linkVideoPlay.setVisibility(View.VISIBLE);
            }else {
                kahooDobaraBinding.linkVideoPlay.setVisibility(View.GONE);
            }
            kahooDobaraBinding.linkPreviewLayout.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (PublicMethods.isYoutubeUrl(finalUrlFromcont)) {
                        if (mContext instanceof ViewKahooFrontFragment){
                            ((ViewKahooFrontFragment) mContext).openYoutubeVideo(position,kahooPostModel,finalUrlFromcont);
                        }else if (mContext instanceof SingleDetailKahooView){
                            ((SingleDetailKahooView) mContext).openYoutubeVideo(position,kahooPostModel,finalUrlFromcont);
                        }else if (mContext instanceof SingleHashTagKahooView){
                            ((SingleHashTagKahooView) mContext).openYoutubeVideo(position,kahooPostModel,finalUrlFromcont);
                        }else if (mContext instanceof DobaraKahooList){
                            ((DobaraKahooList) mContext).openYoutubeVideo(position,kahooPostModel,finalUrlFromcont);
                        }else if (mContext instanceof LikedKahooList){
                            ((LikedKahooList) mContext).openYoutubeVideo(position,kahooPostModel,finalUrlFromcont);
                        }else if (mContext instanceof MyKahooListFrag){
                            ((MyKahooListFrag) mContext).openYoutubeVideo(position,kahooPostModel,finalUrlFromcont);
                        }else if (mContext instanceof ReplyKahooListFrag){
                            ((ReplyKahooListFrag) mContext).openYoutubeVideo(position,kahooPostModel,finalUrlFromcont);
                        }
